...
<div class="z-wrap"><form class="z-form" onchange="form_update(event)" onsubmit="return false"><div><label for="23b1d6ae" title>Debian 版本</label><select id="23b1d6ae" name="release" title><option value="bookworm">Debian 12 (bookworm)</option><option value="bullseye">Debian 11 (bullseye)</option><option value="buster">Debian 10 (buster)</option><option value="testing">testing</option><option value="sid">sid</option></select></div><div><input id="5cac73de" name="src" title="源码源通常用于构建和调试，开启后影响更新速度" type="checkbox"/><label for="5cac73de" title="源码源通常用于构建和调试，开启后影响更新速度">启用源码源</label></div><div><input checked="checked" id="7744b9bd" name="nf" title type="checkbox"/><label for="7744b9bd" title>使用非自由软件源</label></div><div><input id="f8d995c8" name="mirror_security" title="为了更及时地获得安全更新，不推荐使用镜像站安全更新软件源" type="checkbox"/><label for="f8d995c8" title="为了更及时地获得安全更新，不推荐使用镜像站安全更新软件源">强制安全更新使用镜像</label></div></form><pre class="z-code"></pre></div><tmpl z-input="release src nf mirror_security" z-path="/etc/apt/sources.list">
