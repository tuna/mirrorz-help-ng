...
<div class="z-wrap"><form class="z-form" onchange="form_update(event)" onsubmit="return false"><div><label for="b97c6d5f" title>Debian 版本</label><select id="b97c6d5f" name="release" title><option value="bookworm">Debian 12 (bookworm)</option><option value="bullseye">Debian 11 (bullseye)</option><option value="buster">Debian 10 (buster)</option><option value="stretch">Debian 9 (stretch) (EOL)</option><option value="jessie">Debian 8 (jessie) (EOL)</option></select></div><div><label for="8c40bae6" title>服务</label><select id="8c40bae6" name="service" title><option value="pve">Proxmox Virtual Environment (PVE)</option><option value="pbs">Proxmox Backup Server (PBS)</option><option value="pbs-client">Proxmox Backup Client (PBS-CLIENT)</option><option value="pmg">Proxmox Mail Gateway (PMG)</option></select></div></form><pre class="z-code"></pre></div><tmpl z-input="release service" z-path="/etc/apt/sources.list.d/pve-no-subscription.list">
