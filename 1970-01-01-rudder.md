...
<p>首先选择你要安装的版本：</p>
<div class="z-wrap"><form class="z-form z-global" onchange="form_update(event)" onsubmit="return false"><div><label for="c743b4dc" title>Rudder 版本号</label><select id="c743b4dc" name="version" title><option value="6.0">6.0</option><option value="5.0">5.0</option><option value="4.3">4.3</option></select></div></form></div>
<h3>Debian/Ubuntu 用户</h3>
<p>选择你的 Debian/Ubuntu 版本，文本框中内容写进 <code>/etc/apt/sources.list.d/rudder.list</code></p>
<div class="z-wrap"><form class="z-form" onchange="form_update(event)" onsubmit="return false"><div><label for="a5eec238" title>发行版</label><select id="a5eec238" name="release" title><option value="buster">Debian 10 (buster)</option><option value="stretch">Debian 9 (stretch) (EOL)</option><option value="jessie">Debian 8 (jessie) (EOL)</option><option value="focal">Ubuntu 20.04 LTS (focal)</option><option value="bionic">Ubuntu 18.04 LTS (bionic)</option><option value="xenial">Ubuntu 16.04 LTS (xenial) (EOL)</option><option value="trusty">Ubuntu 14.04 LTS (trusty) (EOL)</option></select></div></form><pre class="z-code"></pre></div><tmpl z-input="release" z-path="/etc/apt/sources.list.d/rudder.list">
