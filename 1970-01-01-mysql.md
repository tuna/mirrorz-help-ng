...
<p>首先选择要安装的版本：</p>
<div class="z-wrap"><form class="z-form z-global" onchange="form_update(event)" onsubmit="return false"><div><label for="bc1a4854" title>MySQL 版本号</label><select id="bc1a4854" name="version" title><option value="8.0">8.0</option><option value="5.7">5.7</option><option value="5.6">5.6</option></select></div></form></div>
<h3>Debian/Ubuntu 用户</h3>
<p>再选择你的 Debian/Ubuntu 版本，文本框中内容写进 <code>/etc/apt/sources.list.d/mysql-community.list</code></p>
<div class="z-wrap"><form class="z-form" onchange="form_update(event)" onsubmit="return false"><div><label for="f9b5fab4" title>发行版</label><select id="f9b5fab4" name="deb_release" title><option value="bullseye">Debian 11 (bullseye)</option><option value="buster">Debian 10 (buster)</option><option value="stretch">Debian 9 (stretch) (EOL)</option><option value="jessie">Debian 8 (jessie) (EOL)</option><option value="jammy">Ubuntu 22.04 LTS (jammy)</option><option value="focal">Ubuntu 20.04 LTS (focal)</option><option value="bionic">Ubuntu 18.04 LTS (bionic)</option><option value="xenial">Ubuntu 16.04 LTS (xenial) (EOL)</option></select></div></form><pre class="z-code"></pre></div><tmpl z-input="deb_release" z-path="/etc/apt/sources.list.d/mysql-community.list">
...
<div class="z-wrap"><form class="z-form" onchange="form_update(event)" onsubmit="return false"><div><label for="a8b8ddc2" title>发行版</label><select id="a8b8ddc2" name="rh_release" title><option value="el7">CentOS/RHEL 7</option></select></div></form><pre class="z-code"></pre></div><tmpl z-input="rh_release" z-lang="ini" z-path="/etc/yum.repos.d/mysql-community.repo">
